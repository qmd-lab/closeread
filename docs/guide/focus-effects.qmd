...
Focus effects are all supplied as [attributes on an inline span](https://quarto.org/docs/authoring/markdown-basics.html#ordering-of-attributes) that is attached to a trigger.

:::{.callout-tip appearance="simple"}
Like all Quarto inline spans, focus effects follow a `attribute="value"` syntax: the value must be quoted and `=` separates the value from the attribute with no spaces.
:::
